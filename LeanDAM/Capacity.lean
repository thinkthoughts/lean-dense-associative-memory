...
/--
For two distinct coordinates, exactly one quarter of all Boolean samples
have value `true` at both coordinates.
-/
theorem coordinate_pair_true_true_fiber_card
    (Memory Neuron : Type)
    [Fintype Memory]
    [Fintype Neuron]
    (μ₁ μ₂ : Memory)
    ...
    Fintype.card
          {ω : Omega Memory Neuron //
            ω μ₁ i₁ = true ∧ ω μ₂ i₂ = true} * 4 =
      Fintype.card (Omega Memory Neuron) := by
  classical

  have h12 : ¬ (μ₂ = μ₁ ∧ i₂ = i₁) := by
    intro h
    rcases hdist with hμ | hi
    · exact hμ h.1.symm
    · exact hi h.2.symm
  ...
  have hsplit :
    ...
    simpa [f, add_assoc, add_left_comm, add_comm] using h
